...
public class OdinAgent implements IOdinAgent {
	...
	/**
	 * Probe the agent for a list of VAPs its hosting. This should
	 * only be used by the master when an agent registration to shield
	 * against master failures. The assumption is that when this
	 * is invoked, the controller has never heard about the agent
	 * before.
	 * 
		...
		for (String entry: tableList) {
			...
			try {
				oc = new OdinClient(MACAddress.valueOf(properties[0]),
												InetAddress.getByName(properties[3]),
												MACAddress.valueOf(properties[1]),
												properties[2]);
	...
	public Map<String, Map<String, String>> getRxStats () {
		...
		/*
		 * We basically get rows like this
		 * MAC_ADDR1 prop1:<value> prop2:<value>
	 ...
	 * will also help speedup handoffs.
	 * 
